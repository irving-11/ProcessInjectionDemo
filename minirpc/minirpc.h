...
#define MRPC_MAX_LOCAL_VAR_CHAR_LEN 256 /* Small string optimization length */
...
void mrpc_varchar_create( struct mrpc_varchar* , const char* str , int own, unsigned long len );
...
#define mrpc_val_varchar(val,str,own,len) \
    do { \
        (val)->type = MRPC_VARCHAR; \
        mrpc_varchar_create(&((val)->value.varchar),(str),own,len); \
    } while(0)
