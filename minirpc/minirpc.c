...
static int decode_varchar(void *buffer, size_t length, struct mrpc_val *val)
{
    ...
    else
    {
        ...
        CAST(char *, val->value.varchar.val)
        [str_len] = 0;
    }
...
static size_t encode_varchar(const struct mrpc_varchar *varchar, char *buffer)
{
...
static size_t mrpc_cal_val_size(const struct mrpc_val *val)
{
    switch (val->type)
    {
    case MRPC_UINT:
...
static int mrpc_encode_val(const struct mrpc_val *val, char *buffer)
{
...
static int mrpc_decode_val(struct mrpc_val *val, const char *buffer, size_t length)
{
    int type;
    int ret;
    if (length < 1)
...
static int mrpc_request_parse(void *buffer, size_t length, struct mrpc_request *req)
{
    size_t len;
    size_t cur_pos = 0;
    int ret;
...
static size_t mrpc_cal_response_size(const struct mrpc_response *response)
{
...
static void *mrpc_response_serialize(const struct mrpc_response *response, size_t *len)
{
    ...
    *CAST(char *, data) = CAST(char, response->method_type);
    ...
    /* Transaction ID */
    CAST(char *, data)
    [0] = response->transaction_id[0];
    CAST(char *, data)
    [1] = response->transaction_id[1];
    CAST(char *, data)
    [2] = response->transaction_id[2];
    CAST(char *, data)
    [3] = response->transaction_id[3];
...
static size_t mrpc_cal_request_size(const struct mrpc_request *req)
{
...
static void *mrpc_request_msg_serialize(const struct mrpc_request *req, size_t *len)
{
    ...
    *CAST(char *, data) = req->method_type;
    ...
    /* Transaction ID */
    CAST(char *, data)
    [0] = req->transaction_id[0];
    CAST(char *, data)
    [1] = req->transaction_id[1];
    CAST(char *, data)
    [2] = req->transaction_id[2];
    CAST(char *, data)
    [3] = req->transaction_id[3];
    sz -= 4;
    data = CAST(char *, data) + 4;
    /* Method name */
...
int mrpc_response_parse(void *data, size_t length, struct mrpc_response *response)
{
    int ret;
...
static struct minirpc RPC;

static int MRPC_INSTANCE_NUM = 0;

static void mrpc_request_parse_fail(struct mrpc_conn *conn)
{
...
static int mrpc_do_read(struct net_connection *conn, struct mrpc_conn *rconn)
{
...
static int mrpc_on_conn(int ev, int ec, struct net_connection *conn)
{
...
static int mrpc_on_accept(int ec, struct net_server *ser, struct net_connection *conn)
{
    if (ec == 0)
    {
...
static int mrpc_on_client_do_read(struct net_connection *conn, struct mrpc_client_req *req, void *poll)
{
    if (req->sz == 0)
    {
...
static int mrpc_on_client(int ev, int ec, struct net_connection *conn)
{
...
static void mrpc_poll_handle_response(struct mrpc_res_data *res)
{
    switch (res->tag)
    {
...
static int mrpc_on_poll(int ev, int ec, struct net_connection *conn)
{
...
static void mrpc_stop(int signal)
{
    signal = signal;
    mrpc_interrupt();
}
...
void mrpc_varchar_create(struct mrpc_varchar *varchar, const char *str, int own, unsigned long len)
{
    if (own)
    {
...
/* client function */
static void gen_transaction_id(char transaction_id[4])
{
...
static void mrpc_request_clean(struct mrpc_request *req)
{
    size_t i;
...
static void *mrpc_request_vserialize(size_t *len, int method_type, const char *method_name, const char *par_fmt, va_list vl)
{
    ...
    for (i = 0; par_fmt[i]; ++i)
    {
        if (par_fmt[i] == '%')
        {
            ...
            case 's':
                ...
                mrpc_varchar_create(&(req.par[req.par_size].value.varchar), va_arg(vl, const char *), 0, strlen(va_arg(vl, const char *)));
                break;
            default:
                goto fail;
            }
...
static int mrpc_request_do_send(socket_t fd, const void *data, size_t sz)
{
    size_t offset = 0;
    do
    {
...
static int mrpc_request_do_recv(socket_t fd, struct mrpc_response *resp)
{
    int ret;
